        ...
        <div className="bg-card rounded-3xl border border-[hsl(var(--border))] divide-y divide-[hsl(var(--border))]">
          {products.map((product) => (
            <div key={product.id} className="p-4 flex items-center gap-3">
              ...
              <span className={`px-3 py-1 rounded-full text-xs font-semibold capitalize ${statusStyles[product.status] || "bg-muted"}`}>
                {product.status}
              </span>
              <p className="font-heading text-foreground">{formatStoreMoney(product.price_cents, product.currency)}</p>
              <button type="button" onClick={() => setEditing(product)} className="w-11 h-11 rounded-2xl bg-muted flex items-center justify-center" aria-label={`Edit ${product.name}`}>
                <Pencil className="w-4 h-4" />
              </button>
              <button type="button" onClick={() => deleteProduct(product)} className="w-11 h-11 rounded-2xl bg-muted text-rose-500 flex items-center justify-center" aria-label={`Delete ${product.name}`}>
