...
const PRINT_COSTS = [
  ...
  ...row,
  total: row.base + row.paper + row.packing,
}));
...
const deductibleValue = (expense) => {
  if (expense?.deductible_amount != null) {
    return num(expense.deductible_amount);
  }

  return (
    num(expense?.amount) *
    (num(expense?.deductible_percent ?? 100) / 100)
  );
};
...
function TableRow({ children, last = false }) {
  return (
    <div
      className={`grid grid-cols-12 gap-2 py-2.5 text-sm ${
        last
          ? ""
...
export default function Dashboard() {
  ...
  const activeOrders = useMemo(() => {
    ...
    return orders.filter((order) =>
      trackedSites.includes(displayPlatform(order.platform))
    );
  }, [orders, trackedSites, sitesConfigured]);

  const refresh = async () => {
    await Promise.all([
      reloadOrders({ syncTracker: true }),
      reloadExpenses(),
    ]);
  };
  ...
  const calc = useMemo(() => {
    ...
    const totalSales = activeOrders.reduce(
      (sum, order) => sum + num(order.sale_total),
      0
    );

    const itemsSold = activeOrders.reduce(
      (sum, order) => sum + num(order.quantity),
      0
    );

    const orderCosts = activeOrders.reduce(
      (sum, order) => sum + num(order.total_cost),
      0
    );

    const recordedPurchases = expenses.reduce(
    ...
    const currentOrderCosts = currentOrders.reduce(
      (sum, order) => sum + num(order.total_cost),
      0
    );
    ...
    const platformPerformance =
      platformNames.map((platform) => {
        const rows = activeOrders.filter(
          (order) =>
            displayPlatform(order.platform) ===
            platform
        );
    ...
    const monthlySnapshot = monthKeys.map(
      (key) => {
        const monthOrders =
          activeOrders.filter(
            (order) =>
              (order.sale_date || "").slice(
                0,
                7
              ) === key
          );
        ...
        const costs = monthOrders.reduce(
          (sum, order) =>
            sum + num(order.total_cost),
          0
        );
    ...
    <div className="space-y-6 md:space-y-7">
      ...
      <div className="hidden md:flex items-start justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          <div className="rounded-2xl border border-[hsl(var(--border))] bg-card px-4 py-2.5 flex items-center gap-2 text-sm font-medium shadow-sm">
          ...
          <Link
            to="/account"
            className="w-11 h-11 rounded-2xl bg-card border border-[hsl(var(--border))] flex items-center justify-center shadow-sm"
            aria-label="Account"
          >
      ...
      <section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-4">
          <MetricCard
            tone="lavender"
            label="Total Sales"
            value={formatMoney(calc.totalSales)}
            loading={loading}
          />
          ...
          <MetricCard
            ...
            value={formatMoney(
              calc.estimatedNetProfit
            )}
            loading={loading}
          />

          <div className="col-span-2 md:col-span-1 md:[&>div]:h-full xl:col-span-1">
            <MetricCard
              tone="yellow"
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        <MetricCard
          tone="lavender"
          label="Order Costs"
          value={formatMoney(calc.orderCosts)}
          loading={loading}
        />
        ...
        <MetricCard
          tone="yellow"
          label={`${currentLabel} Net`}
          value={formatMoney(calc.currentNet)}
          loading={loading}
        />
      </section>

      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 md:gap-5">
        <section className="bg-card rounded-3xl p-5 md:p-6 border border-[hsl(var(--border))]">
          <h2 className="font-heading text-lg mb-4">
            Platform Performance
          </h2>

          {calc.platformPerformance.length ===
          0 ? (
        ...
        <section className="bg-card rounded-3xl p-5 md:p-6 border border-[hsl(var(--border))]">
          <h2 className="font-heading text-lg mb-1">
            Current Art Print Costs
          </h2>

          <p className="text-xs text-muted-foreground mb-3">
            Base item + paper & ink + packing
          </p>
            ...
            {PRINT_COSTS.map(
              (row, index) => (
                ...
                >
                  ...
                  <span className="col-span-2 text-right">
                    {formatMoney(row.paper)}
                  </span>

                  <span className="col-span-2 text-right">
                    {formatMoney(row.packing)}
                  </span>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 md:gap-5">
        <section className="bg-card rounded-3xl p-5 md:p-6 border border-[hsl(var(--border))]">
          ...
          ) : calc.monthlySnapshot.length ===
            0 ? (
            <EmptyRow text="No monthly data yet" />
          ) : (
            <div className="space-y-3">
              {calc.monthlySnapshot.map(
                (row) => (
                  <div
                    key={row.key}
              ...
              {calc.expenseMix.map(
                (row, index) => (
                  ...
                  >
                    ...
                    <span className="font-heading text-sm shrink-0">
                      {formatMoney(
                        row.amount
                      )}
                    </span>
